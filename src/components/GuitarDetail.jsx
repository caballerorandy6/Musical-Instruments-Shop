...
const GuitarDetail = () => {
  ...
  const getGuitar = async () => {
    try {
      const url = `http://127.0.0.1:1337/api/guitars?filters[url]=${guitarURL}&populate=image`;
      const { data: guitar } = await axios(url);
      setGuitar(guitar.data[0].attributes);
      setGuitarId(guitar.data[0].id);
    } catch (error) {
      console.log(error);
    }
  };
